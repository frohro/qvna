...
void SettingsDialog::initializeUISettings()
{
...
void SettingsDialog::readSettings()
{
    ...
    currentSettings.name = settings.value("name",currentSettings.name).toString();
    currentSettings.baudRate = settings.value("baud",currentSettings.baudRate).toInt();
    currentSettings.stringBaudRate = QString::number(currentSettings.baudRate);
    currentSettings.dataBits = static_cast<QSerialPort::DataBits>(
                settings.value("dataBits",currentSettings.dataBits).toInt());
    currentSettings.stringDataBits = QString::number(currentSettings.dataBits);
    currentSettings.parity = static_cast<QSerialPort::Parity>(
                settings.value("parity",currentSettings.parity).toInt());
    currentSettings.stringParity = QString::number(currentSettings.parity);
    currentSettings.stopBits = static_cast<QSerialPort::StopBits>(
                settings.value("stopBits",currentSettings.stopBits).toInt());
    currentSettings.stringStopBits = QString::number(currentSettings.stopBits);
    currentSettings.flowControl = static_cast<QSerialPort::FlowControl>(
                settings.value("flowControl",currentSettings.flowControl).toInt());
    currentSettings.stringFlowControl = QString::number(currentSettings.flowControl);
    currentSettings.localEchoEnabled =
            settings.value("localEchoEnabled",currentSettings.localEchoEnabled).toBool();
}
...
void SettingsDialog::writeSettings()
{
    ...
    settings.setValue("baud",currentSettings.baudRate);
...
void SettingsDialog::refreshNShow(void)
{
    fillPortsInfo();
    readSettings();
    initializeUISettings();
